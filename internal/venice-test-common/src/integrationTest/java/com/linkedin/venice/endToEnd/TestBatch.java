...
public abstract class TestBatch {
  ...
  public void testKafkaInputBatchJob() throws Exception {
    ...
    String storeName = testBatchStore(
        ...
        properties -> {},
        validator);
    assertInvalidPerStoreMetrics(storeName, "assembled_record_value_size_in_bytes");
  ...
  private List<Double> getPerStoreMetricValues(String storeName, String sensorName) {
    ...
    return Arrays.asList(
        MetricsUtils.getMin(metricName + ".Min", veniceCluster.getVeniceServers()),
        MetricsUtils.getMax(metricName + ".Max", veniceCluster.getVeniceServers()),
        MetricsUtils.getAvg(metricName + ".Avg", veniceCluster.getVeniceServers()));
  }

  private void validatePerStoreMetrics(String storeName, String sensorName, double minValueSize, double maxValueSize) {
    getPerStoreMetricValues(storeName, sensorName).forEach(value -> {
      Assert.assertTrue(value >= minValueSize, "Metric value should be at least " + minValueSize);
      Assert.assertTrue(value <= maxValueSize, "Metric value should be at most " + maxValueSize);
    });
  }

  private void assertInvalidPerStoreMetrics(String storeName, String sensorName) {
    getPerStoreMetricValues(storeName, sensorName).forEach(value -> {
  ...
  public void testLargeValues() throws Exception {
    ...
    validatePerStoreMetrics(storeName, "assembled_record_value_size_in_bytes", 1, MAX_RECORD_VALUE_SIZE);
  }
